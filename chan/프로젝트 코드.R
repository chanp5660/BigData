# cd C:\Users\korea\Downloads\bigdatadel
...
Retrival <- function(item){
  ...
  if(item!="default"){ # 아무것도 적지 않았을때는 그냥 그 창에서 실행 
    Sys.sleep(1)
...
item = as.vector(unlist(read.csv("C:/Users/korea/Downloads/bigdatadel/item.txt",header=F))) # 아이템 정보 
database = Database("크레이지아케이드",c("happy","ddd"),item) # 게임명-서버명-아이템명 데이터화
database = Database("로스트아크","루페온","default") # 게임명-서버명-아이템명 데이터화

GameServer("로스트아크","루페온")#루페온 들어가기.
