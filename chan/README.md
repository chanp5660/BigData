...
```
   효과 : 어떤 게임이 거래수/총거래금액 가장 많은지 비교 가능 -> 아이템매니아가 독점하고 있기 때문에 현금거래량을 추측할 수 있다.
   유저가 몇명인지는 파악할 수 없다.
```
...
```
   ...
   예를 들어서 
       ...
       인기 게임 ‘리니지’ 같은 경우
                               - 1개의 서버 당 하루에 평균 200건 정도 판매글이 올라온다.
                               - 서버가 35개나 되므로 하루에 7000건의 판매글이 올라온다고 할 수 있다.
...
#### 1. 가격에 대한 기초통계량( 평균 , 분산 ) (아이템마다 큰 차이가 있을 수 있음)

#### 2. 그래프 

- 물품별 거래 건수 파악(TOP n)
- 거래건수(거래 완료)가 많은 아이템들에 대한 가격 변동
- 아이템별 평균(최저가) 가격의 시계열 그래프, 전체 아이템 평균(최저가) 가격의 시계열 그래프
